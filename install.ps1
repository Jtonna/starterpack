...
.PARAMETER InitBeads
    Auto-initialize Beads if not already present. Requires bd and dolt on PATH.

.PARAMETER NoCommit
    Skip the auto-commit step after installation.

.EXAMPLE
    # Download and run with flags (recommended)
    irm https://raw.githubusercontent.com/Jtonna/starterpack/main/install.ps1 -OutFile $env:TEMP\sp.ps1; & $env:TEMP\sp.ps1
    ...
    irm https://raw.githubusercontent.com/Jtonna/starterpack/main/install.ps1 -OutFile $env:TEMP\sp.ps1; & $env:TEMP\sp.ps1 -Version v1.2.0
#>
[CmdletBinding()]
param(
    [string]$Version,
...
# Support env var overrides (for backward compat with iex pipeline usage)
...
try {
    ...
    if (-not $beadsInitialized) {
        if (-not $InitBeads) {
            ...
            Write-Host '    irm https://raw.githubusercontent.com/Jtonna/starterpack/main/install.ps1 -OutFile $env:TEMP\sp.ps1; & $env:TEMP\sp.ps1 -InitBeads' -ForegroundColor Cyan
            Write-Host ""
            return
        }
    ...
    if (Test-Path (Join-Path $PWD ".git")) {
        ...
        if (Get-Command "bd" -ErrorAction SilentlyContinue) {
            ...
            if ($LASTEXITCODE -eq 0) {
                $hooksMsg = "pre-commit, post-merge, pre-push, post-checkout, prepare-commit-msg"
                Write-Host "  [ok] Beads hooks installed ($hooksMsg)" -ForegroundColor Green
            } else {
    ...
    Write-Host ""
    $filesMsg = "$copied files"
    Write-Host "Installed starterpack $resolvedVersion ($filesMsg)" -ForegroundColor Green
    if ($skipped -gt 0) {
    ...
    if ($warnings.Count -gt 0) {
        ...
        Write-Host "Ready to go. Start the orchestrator with: claude" -ForegroundColor Green
    }
}
finally {
    # Cleanup temp files
